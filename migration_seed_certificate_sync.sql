-- Sync public.certificates demo / catalogue rows with current seed.sql payloads.
-- Safe to run after an older seed: updates existing rows that were skipped by ON CONFLICT DO NOTHING.
--
-- Rules:
--   • Legacy four codes: UPDATE details (Ipamorelin also refreshes status = pending).
--   • Seven catalogue codes + twelve expansion-pack codes: INSERT new rows OR UPDATE in place only when user_id IS NULL.
--
-- Run in Supabase SQL Editor (or psql). Re-running is safe for catalogue upserts.

-- ---------------------------------------------------------------------------
-- Legacy demo rows — replace details JSON only
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Catalogue rows (seven) — insert missing or refresh catalogue/demo-owned rows
-- ---------------------------------------------------------------------------
...
set
  ...
  hplc_purity = excluded.hplc_purity,
  lcms_ppm = excluded.lcms_ppm,
